...
import org.vanilladb.core.sql.VarcharType;
...
public class ExplainPlan implements Plan {
	...
	public ExplainPlan(Plan p) {
		this.p = p;
		schema.addField("query-plan", VarcharType.VARCHAR(500));
		hist = p.histogram();
	}
	...
	public Scan open() {
		Scan s = p.open();
		return new ExplainScan(s, schema, explain(tab_cnt));
	}
	...
	public String explain(int tab_cnt) {
		String tab = "";
		for(int i=0; i<tab_cnt; i++) {
			tab += '\t';
		}
		tab_cnt++;
		return "\r\n" + tab + p.explain(tab_cnt);
	}
